...
void CImageViewerDlg::OnOK()
{
	...
	if(!m_bStartRand)		// 确保当前的随机数没有被选中过，并且不是特殊值
	{
		m_nCur+=1;
		bool bFault = true;
		while(bFault)
		{
			bFault = false;
			for(int i = 0; i < m_nCur-1; i++)	// 如果当前值已经在列表当中，则重新选择
			{
				if(m_GetNum[m_nCur-1] == m_GetNum[i])
				{
			...
			for(int i = 0; i < m_nSpecialNum; i++)	// 如果当前值是特殊值的话，重新选择
			{
				if(m_GetNum[m_nCur-1] == m_nvSpecial[i])
				{
					m_GetNum[m_nCur-1] = (rand() % (m_nMax-m_nMin+1)) + m_nMin ;
					bFault = true;
					Invalidate(true);
		...
		if (m_nCur % 2 == 0 && m_nCurSpecial < m_nSpecialNum)	// 选择特殊号码
		{
			m_GetNum[m_nCur - 1] = m_nvSpecial[m_nCurSpecial++];
		}
	}
	else
	{
